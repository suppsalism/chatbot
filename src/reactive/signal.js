...
function notify(subscribers) {
  ...
  // Array.from, NOT [...subscribers]. The build transpiles array spread in
  // loose mode, where `[...x]` becomes `[].concat(x)` — and concat only
  // flattens arrays, so a Set comes through as a single element rather than
  // its members. That silently turned this snapshot into `[theSet]`, so no
  // effect ever re-ran in the published bundles while every source-level test
  // passed. Array.from is not rewritten and iterates properly.
  // Snapshotting also matters on its own: an effect re-subscribes as it runs,
  // which mutates `subscribers` mid-iteration.
...
export function createRoot(fn) {
  ...
  function dispose() {
    // Array.from for the same reason as in notify() — see the note there.
    for (const effect of Array.from(owner.effects).reverse()) effect.dispose();
