...
**Companion to:** `chatbot-core-spec.md`, which defines _what_ the package does.
This document defines _how to build it_ — file structure, coding conventions,
...
- **`lib/mount/widget.js`** — `iframe.js` prepares a second _document_; it never
...
export class Composer {
  ...
  on(node, event, handler) {
    // ALWAYS bind through this
    node.addEventListener(event, handler);
    this.listeners.push([node, event, handler]);
  }

  build() {
    /* create this.element, bind via this.on() */
  }
...
export function createChatbot(options) {
  ...
  return createRoot((dispose) => {
    ...
    const bot = {
      ...
      updateConfig: (patch) => {
        /* re-resolve, re-apply both roots, emit */
      },
      getState: () => ({/* … */}),
      destroy: () => {
        view.destroy();
