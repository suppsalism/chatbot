...
The CDN URL is a view of the npm tarball; jsDelivr and unpkg serve _from_ the
...
class Composer {
  ...
  build() {
    /* create this.element, bind listeners via this.on() */
  }
  ...
  customElements.define(
    ...
    class extends HTMLElement {
      static get observedAttributes() {
        return CONFIG_SCHEMA.map((f) => f.attribute);
      }
      connectedCallback() {
...
test('the window global exposes exactly the module exports', () => {
  ...
  expect(
    Object.keys(window.SsChat)
      .filter((k) => !cdnOnly.includes(k))
      .sort()
...
A genuinely separate document is warranted when the _audience_ differs, not when
