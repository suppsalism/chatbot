...
_before_ any implementation change, per the agreed sequencing - everything
...
anything about where it's being called from. Merely _importing_
...
Blocking a user's _outgoing_ message (via `beforeSubmit`) is a
well-understood pattern (spam/PII filters); silently dropping the AI's
_response_ via the same falsy-return convention felt like a much easier
...
| `feedback:submitted` | _(reserved, gated by `collectUserFeedback`; no UI yet)_       | `{ replyTo, rating: 'up' \| 'down', comment?: string }`  |
| `lead:submitted`     | _(reserved; no UI yet)_                                       | `{ fields: Record<string, string> }`                     |
